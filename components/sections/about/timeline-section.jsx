...
const TimelineSection = () => {
  ...
  useEffect(() => {
    const updateScrollProgress = () => {
      ...
      const scrollPosition = scrollTop - timelineOffsetTop + windowScrollHeight / 2;
      const scrollPercentage = Math.max(0, Math.min(1, scrollPosition / timelineHeight));
      ...
      iconsRefs.current.forEach((iconRef, index) => {
        ...
        if (scrollPosition >= iconPosition && scrollPosition <= iconPosition + iconHeight) {
        ...
        <div className="timeline-wrapper">
          ...
          <div className="timeline-wrap relative">
            <div
              ...
              style={{
                backgroundColor: isScrolling ? "rgb(128,0,128)" : "rgb(192,192,192)",
              ...
              {timelineData.map((item, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    } ${scrollProgress >= index * (100 / timelineData.length) ? 'text-purple' : 'text-black'}`}
                  >
                    ...
                    <div className="flex items-center justify-center w-10 h-10 bg-white md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                      <div dangerouslySetInnerHTML={{ __html: item.icon }} ref={ref => iconsRefs.current[index] = ref} className={`${scrollProgress >= index * (130 / timelineData.length) ? 'text-purple' : 'text-black'}`}/>
                    </div>
                  </div>
                  {/* Card */}
                  <div
                  ...
                  >
                    <div className="mb-1">
                      <p className="small-para font-bold manrope">{item.year}</p>
                      <h3 className="font-bold text-slate-900 manrope">{item.title}</h3>
                    </div>
